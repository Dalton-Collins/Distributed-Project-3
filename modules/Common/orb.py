...
class Stub(object):
    ...
    def _rmi(self, method, *args):
        ...
        json_request = ''.join((json.dumps({'method': method,
                                            'args': args}), '\n'))
        reply_buffer = ''
        ...
        try:
            ...
            while True:
                ...
                if not data:
                    break
                reply_buffer = ''.join((reply_buffer, data))
                data = ''
            result = json.loads(reply_buffer).get('result')

            if not result:
                result = json.loads(reply_buffer).get('error')
...
class Request(threading.Thread):
    ...
    def run(self):
        try:
            '''
            This is the same as the request from the last lab
            except for the getattr business
            '''
            ...
            output = ''
            try:
                method = getattr(self.owner, request_dict.get("method"))
                result = method(*request_dict.get("args"))
                output = ''.join(json.dumps(({"result": result}), "\n"))

            # TypeError if arguments are wrong for method or request is not a
            # string ValueError if request is not valid json AttributeError if
            # requested method does not exist
            except Exception as e:
                output = ''.join(json.dumps({"error": {"name": type(e).__name__,
                                                       "args": e.args}}))
        ...
        # IOException if connection dies
        except Exception as e:
            logging.info("The connection to the caller has died:")
            logging.info("\t{}: {}".format(type(e), e))
        finally:
            self.conn.close()
...
class Skeleton(threading.Thread):
    ...
    def run(self):
        '''
        Same as the main loop in the server.py code from last project
        '''
